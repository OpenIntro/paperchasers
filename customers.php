...
-->
  ...
  //print_r($_POST);
					if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset( $_GET['u'])=='edit'){
						...
						//update
                        $sql = "UPDATE addcustomer SET fname='".$_POST["1"]."', cname='".$_POST["2"]."',email='".$_POST["3"]."',
						pnumber='".$_POST["4"]."', saddress= '".$_POST["5"]."',ccity='".$_POST["6"]."', state='".$_POST["7"]."',zcode='".$_POST["8"]."',
						note='".$_POST["9"]."'
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1>
                        Customers <a href="customerAdd.php"><button class="btn bg-job btn-flat text-white btn-hover">Add New Customer</button></a>
                    </h1>
                    <ol class="breadcrumb">
...
$sql ="SELECT * FROM `jobdetail` where jcid = ".$row["customerid"].""; 

$resultt = $conn->query($sql);
$tmptotoalj = $resultt->num_rows;

$sql ="SELECT * FROM `jobdetail` where jcid = ".$row["customerid"]." && jtype != 'archive' "; 
$resultt = $conn->query($sql);
$tmpactivej = $resultt->num_rows;
    
		?>
		<tr>
                                                <input type="hidden" name="tmp" class="tmp" id="tmp" value= <?php echo $row["customerid"];?> />
